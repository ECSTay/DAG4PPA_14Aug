---
title: "DAG 4 - reworking - Med S Low R Low SP"
author: "Evelyn Tay"
date: "`r Sys.Date()`"
output: 
...
reaction <- function(A) {if (A > 0) {S <- rbinom(1,1,0.3)} else {S <- rbinom(1, 1,0.45)}}
...
response <- function(dat) 
  {A = dat[1]
  S = dat[2]
  if( A > 0 & S > 0) {R <- rbinom(1, 1, 0.23)}
  else if( A > 0 & S < 1 )  {R <- rbinom(1, 1,0.15)} 
  else if( A < 1 & S > 0 ) {R <- rbinom(1,1, 0.15)}
  else  {R <- rbinom(1,1, 0.10)} 
  return(R)
  }
...
reaction2 <- function(A2) {if (A2 > 0) {S2 <- rbinom(1, 1,0.3)} else {S2 <- rbinom(1, 1,0.45)}}
